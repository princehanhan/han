...
<c:forEach items="${pagers.datas}" var="b" varStatus="l">
    <ul class="proSelect" style="border-top: none;">
        ...
        <a href="##"><li><c:if test='${b.isYj == 1}'>${b.goods.user.phone}</c:if></li></a>
            <li>
                <c:if test='${b.goods.isSd == 1 && b.jyUser.zd == 1 && b.isYj == 1}'><a href="##" onclick="edit(${b.goods.id}, ${b.id})">归还</a></c:if>
                <c:if test='${b.isYj == 1 && b.goods.isSd == 0 && b.jyUser.zd == 0}'><a href="##" onclick="shou(${b.goods.id})">确认收到图书</a></c:if>
                <c:if test='${b.isYj == 2}'><a href="##">已经归还图书</a></c:if>
            </li>
        <li><c:if test='${b.isYj == 1}'><a href="shopping_edit.do?id=${b.goods.id}">现在去</a></c:if></li>
        <div class="clears"></div>
    </ul><!--proSelect/-->
    </c:forEach>
</form><!--proDingzhi/-->
