...
<div id="Member_Ratings">
    <div class="d_Confirm_Order_style">
        <form method="post" action="goods_list.do">
...
<div class=" clearfix" id="advertising">
    <div class="Ads_list" style="margin-left: 0">
       ...
       <span class="l_f">
        <a href="goods_add.do" id="ads_add" class="btn btn-warning"><i class="fa fa-plus"></i> 添加图书</a>
       </span>
        </div>
        <div class="Ads_lists">
            <table class="table table-striped table-bordered table-hover" id="sample-table">
                ...
                <tr>
                    ...
                    <th width="100px">图书所有人</th>
                    <th width="100px">是否借阅</th>
                    <th width="80">借阅次数</th>
                    <th width="80">图书状态</th>
                    <th width="80">图书审核</th>
                ...
                <tr>
                    <td>${c.itemTypeSecondary.name}</td>
                    <td><span class="ad_img"><img src="${ctx}${c.fileName1}" style="width: 200px; height: auto"/></span></td>
                    <td><a href="goods_detail.do?id=${c.id}">${c.name}</a></td>
                    <td>${c.user.name}</td>
                    <td class="td-status"><span class="label label-success radius"><c:if test="${c.isJy == 0}">未借阅</c:if><c:if test="${c.isJy == 1}">已借阅</c:if></span></td>
                    <td class="td-status"><span class="label label-success radius">${c.count}</span></td>
                    <td class="td-status"><span class="label label-success radius"><c:if test="${c.isSj == 0}">上架</c:if><c:if test="${c.isSj == 1}">下架</c:if></span></td>
                    <td class="td-status"><span class="label label-success radius">
                         <c:if test="${c.isSh == 0}">未审核</c:if>
                        <c:if test="${c.isSh == 1}">已审核</c:if>
                    </span></td>

                    <td class="td-manage">
                        <a title="<c:if test="${c.isSj == 0}">下架</c:if><c:if test="${c.isSj == 1}">上架</c:if>"  onclick="sj(${c.id})" class="btn btn-xs btn-warning"><i class="fa fa-dashboard bigger-120"></i></a>
                        <a title="编辑"  href="goods_edit.do?id=${c.id}" class="btn btn-xs btn-info"><i class="fa fa-edit bigger-120"></i></a>
        ...
        <!-- 分页开始 -->
        <div class="pagelist">
            <pg:pager url="${ctx}/goods_list.do" maxIndexPages="5" items="${pagers.total}" maxPageItems="15"
                      export="curPage=pageNumber">
                <pg:param name="name" value="${bean.name}"/>
                <pg:param name="itemTypeSecondary.id" value="${bean.itemTypeSecondary.name}"/>
                <pg:last>
