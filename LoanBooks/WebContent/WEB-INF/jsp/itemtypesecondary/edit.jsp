...
<body>
<form action="itemtypesecondary_update.do" method="post" class="form form-horizontal" id="form-user-add">
        ...
        <div class="formControls ">
            <select name="itemType.id" id="names" style=" width:150px">
                ...
                <c:forEach items="${pagers.datas}" var="c" varStatus="l">
                    <option value="${c.id}" <c:if test="${c.id == bean.itemType.id}">selected="selected"</c:if>>${c.name}</option>
                </c:forEach>
            </select>
        </div>
    </div>
...
<script>
    ...
    function sub() {
        ...
        if (names == null || names == "") {
            ...
            AJAX('itemtypesecondary_update.do', "更新成功", "form-user-add");
        }
    }
</script>
</body>
