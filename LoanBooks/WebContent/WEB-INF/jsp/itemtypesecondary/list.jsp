...
<div class="page-content clearfix">
    <div id="Member_Ratings">
        <div class="d_Confirm_Order_style">
            <form method="post" action="itemtypesecondary_list.do">
    ...
    <div class="border clearfix">
       <span class="l_f">
        <a href="##" onclick="add()" id="member_add" class="btn btn-warning"><i class="icon-plus"></i>添加二级分类</a>
       </span>
    </div>
    ...
    <div class="margin clearfix">
        <div class="compete_list">
            <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                ...
                <c:forEach items="${pagers.datas}" var="c" varStatus="l">
                    <tr>
                        <td class="hidden-480">${c.itemType.name}</td>
                        <td class="hidden-480">${c.name}</td>
                        <td class="hidden-480">
                            <a title="编辑" onclick="edit(${c.id})" href="javascript:;" class="btn btn-xs btn-info"><i class="fa fa-edit bigger-120"></i></a>
                                <a title="删除" href="javascript:;" onclick="Competence_del(${c.id})" class="btn btn-xs btn-warning"><i class="fa fa-trash  bigger-120"></i></a>
                        </td>
                    </tr>
                </c:forEach>
                </tbody>
        ...
        <!-- 分页开始 -->
        <div class="pagelist">
            <pg:pager url="${ctx}/itemtypesecondary_list.do" maxIndexPages="5" items="${pagers.total}" maxPageItems="15"
...
<script type="text/javascript">
    function edit(id) {
        layer.open({
            type: 2,
            area: ['410px', '300px'],
            content: ['itemtypesecondary_edit.do?id=' + id, 'no']  //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
        });
    ...
    function add() {
        layer.open({
            type: 2,
            area: ['410px', '300px'],
            content: ['itemtypesecondary_add.do', 'no']  //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
        });
    ...
    function Competence_del(id) {
        layer.confirm('确认要删除吗？', function () {
            $.ajax({
                cache: true,
                type: "post",
                url: "itemtypesecondary_delete.do",
                data: {id: id},// 你的formid
                async: false,
